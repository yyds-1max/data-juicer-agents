...
class RecipeManager:
    ...
    def __init__(self, hub_dir: str, all_ops_config: dict):
    ...
    def _load_all_recipes(self) -> list[Recipe]:
        ...
        loaded_recipes = []
        if not os.path.isdir(self.hub_dir):
            logger.warning(
                f"Recipes directory not found: {self.hub_dir}, Auto loading..."
            )
            self._clone_recipes_from_github()

        for root, _, files in os.walk(self.hub_dir):
            if "sandbox" in root:
                continue
    ...
    def _clone_recipes_from_github(self):
        ...
        try:
            ...
            _ = subprocess.run(
                ["git", "clone", repo_url, self.hub_dir],
                capture_output=True,
                text=True,
                check=True,
            )

            logger.info(f"Successfully cloned recipes to {self.hub_dir}")
        except subprocess.CalledProcessError as e:
            logger.error(f"Failed to clone repository: {e.stderr}")
            logger.warning(
                f"Continuing with original recipes directory: {self.hub_dir}"
            )
    ...
    def _parse_and_convert(self, filepath: str, filename: str) -> Recipe | None:
        ...
        try:
            ...
            if (
                not raw_recipe
                or "process" not in raw_recipe
                or not isinstance(raw_recipe["process"], list)
            ):
            ...
            for op_entry in raw_recipe["process"]:
                if not isinstance(op_entry, dict) or len(op_entry) != 1:
                    logger.warning(
                        f"Skipping malformed entry in {filename}: {op_entry}"
                    )
                    continue
                ...
                logger.info(
                    f"Converting operator '{op_name}', args: {op_args_from_recipe}"
                )

                if op_name not in self.all_ops:
                    logger.warning(
                        f"Recipe '{filename}' uses operator '{op_name}' which is not in all_ops.yaml. Skipping this operator."
                    )
                    continue
                ...
                if base_op_config.get("args") and op_args_from_recipe:
                    for arg_name, arg_value in op_args_from_recipe.items():
                        if arg_name in base_op_config["args"]:
                            ...
                            logger.warning(
                                f"In recipe '{filename}', operator '{op_name}' has an unknown argument '{arg_name}'. Ignoring."
                            )
