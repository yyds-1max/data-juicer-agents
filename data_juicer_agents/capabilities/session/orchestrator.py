...
class DJSessionAgent:
    ...
    def _session_sys_prompt(self) -> str:
        ...
        return (
            ...
            "For complex navigation VLA data processing requests, call `vla_run_workflow` exactly once with parsed date/segments/scene_mode/approve/dry_run. "
            "Do not call atomic VLA tools unless recovering a failed stage or user asks for a specific inspection. "
            "legacy single ReAct VLA tool chain is only for manual recovery, targeted inspection, or single-tool execution. "
            "A suitable CLI entry is: djx vla-workflow run --scenario navigation_vla --date <YYYYMMDD> --segments <all-or-segment-list> --scene-mode out --approve. "
            "Use `dry-run` only when the user explicitly requests a preview/rehearsal or asks not to execute. "
            ...
            "Atomic recovery chain: vla_inspect_raw_date -> vla_check_runtime -> vla_prepare_raw_temp -> vla_extract_and_sync -> "
            "vla_list_clip_segments -> vla_prepare_finish_dataset -> vla_build_noobscenes_inputs -> "
            "vla_run_manual_box_annotation -> vla_run_tracking -> vla_prepare_gridmap -> "
            "vla_run_projection_and_trajectory -> vla_validate_outputs. "
            "Always keep the Agent Python runtime isolated from the legacy data-processing runtime. Legacy Python scripts must run through "
            "the configured Python 3.8 data runtime, not the Agent Python 3.10+ process. Treat gen_box.py as a human-in-the-loop GUI checkpoint: "
            "launch it, wait for the user to finish annotation, inspect generated YAML files, then continue only when outputs are present or the user chooses how to proceed. "
            "Persist and report the VLA run log directory for every pipeline run.\n"
