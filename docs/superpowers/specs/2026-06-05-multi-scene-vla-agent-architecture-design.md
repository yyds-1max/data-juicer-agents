...
2. 引入 Plan-Agent，采用ReAct的工作范式，由它根据用户目标、场景模板、工具能力目录和数据画像生成
   结构化处理计划。
3. 约束 Plan-Agent，避免其凭空规划。Plan-Agent 必须在“场景模板 + 工具能力目
   录 + 数据画像”的约束下规划。
4. 引入 Executor-Agent，采用ReAct的工作范式，由它根据 Plan-Agent 生成并通过校验的计划逐步执行。
...
### 需要具体讨论collect_required_info应该判断用户都提供哪些信息
...
Executor-Agent 不应该重新设计主流程。必须按照当前步骤来调用工具，不能捏造工具。如果执行过程中发现 plan 已经不适用，它
...
Plan-Agent 只能使用安全工具：
目前暂定这些工具，具体为Plan-Agent设计什么工具要在开工时讨论。
```text
只读 inspection 工具
list/discovery 工具
validate 工具
...
## 质检和回流
当前不实现该功能，以下关于质检和回流的讨论只做未来功能扩展参考：
