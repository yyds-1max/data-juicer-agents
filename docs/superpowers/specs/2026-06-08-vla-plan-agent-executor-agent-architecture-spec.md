...
```json
{
  ...
  "extraction_rationale": [
    ...
    "topic 名称与 legacy 默认 topic 不完全一致，因此标记为 custom_topics。"
  ],
  "used_for": [
...
```json
{
  ...
  "stage_variants": {
    ...
    "gridmap_processing": {
      ...
      "reason": "raw has no gridmap topic, but an existing grid_map artifact can be transformed with cp_gridmap semantics into the projection input location",
...
topics.topic_schema：topic 组合类型，如 go2w_current_topics、u_legacy_topics、custom_topics、unknown_topics，决定 extract/sync 工具变体。
...
3.Topic schema
判断 topic 组合属于 u_legacy_topics、go2w_current_topics、shanmao_ins_topics、custom_topics 还是 unknown_topics，并产出 topic 到 canonical dir 的映射。
...
gridmap_processing
  ...
  copy_existing_artifact 不是原样复制，必须复刻服务器 cp_gridmap.py 的 JSON 坐标转换语义。
...
projection_and_trajectory
  根据 gridmap 策略和可用脚本选择 cjl_with_gridmap 或 cjl_0525_with_gridmap。
  定位来源由 data_profile.localization 表达，不塞进 projection variant 名。
...
```text
如果 topic_schema = unknown_topics：
...
{
  "extract_and_sync": {
    "topic_schema_support": ["u_legacy_topics", "go2w_current_topics"],
    "supports_custom_topic_mapping": false,
  ...
  "gridmap_processing": {
    ...
    "output_contract": "grid_map is available at the projection input path before projection_and_trajectory starts; JSON files have cp_gridmap coordinate transform applied"
  },
  "prepare_finish_dataset": {
...
`reason`。
导航场景最终要求 `grid_map`。如果跳过 `gridmap_processing`，原因只能是 `grid_map`
已经存在于 projection 所需输入位置，并且必须记录对应 evidence。
...
根据真实 topic 组合识别 u_legacy_topics、go2w_current_topics、shanmao_ins_topics、custom_topics、unknown_topics，并产出 canonical 映射。
...
只检查 raw topic、clip/sync、finish temp、finish final 是否存在 `grid_map`。
本工具不查询 capability catalog，不判断 `pointcloud_to_gridmap` 是否可用，也不写
`stage_variants.gridmap_processing`。
填充事实：gridmap.raw_gridmap_topic_present、gridmap.available_gridmap_artifacts、
gridmap.projection_input_gridmap_ready、gridmap.gridmap_source、evidence。
其中 gridmap.gridmap_source 只能根据文件事实填 `raw_topic`、`existing_gridmap_artifact` 或
`unknown`；是否可从点云生成、是否阻塞、以及应选择哪个 gridmap_processing variant，
由 Plan-Agent 结合 Tool capability catalog 决定。
...
2. **测试策略**
    - 如何构造 20270515 raw 无 gridmap topic、clip/sync 无现成 grid_map、必须走点云生成的样例；
    - 如何构造 20270605 custom_topics 样例；
...
19. `gridmap_processing` 是导航 workflow 中可执行的独立 stage，放在 `run_tracking` 之后、`projection_and_trajectory` 之前；`vla_prepare_gridmap` 负责按 cp_gridmap 坐标转换语义准备已有 grid_map artifact，或从点云生成 grid_map 后再准备到 projection 输入位置，`projection_and_trajectory` 只消费已准备好的 grid_map，不负责生成或复制。
